...
| `setup_input` | no | `""` | JSON options for the `setup` command (see schema below) |
| `build_input` | no | `""` | JSON options for the `build` command (see schema below) |
...
All fields are optional. When neither `go_version` nor `go_version_file` is supplied, `go.mod` is detected automatically. Omit `setup_input` entirely for standard usage.
...
All fields are optional. Omit `build_input` entirely to run `go build .` in the current directory.
...
Installs Go from go.dev. When `go.mod` is present in the working directory, the version is read from it automatically — no `setup_input` needed. The module cache at `~/go/pkg/mod` and the Go installation are saved and restored via the Actions cache by default.
...
jobs:
  build:
    ...
    steps:
      ...
      # go.mod is auto-detected; setup_input can be omitted entirely
...
```yaml
      - uses: bshore/gha/actions/go@go-v0.4.1
        with:
          token: ${{ secrets.GITHUB_TOKEN }}
          command: setup
          setup_input: '{"go_version_file": "go.mod", "cache_dependency_path": "go.sum"}'
```

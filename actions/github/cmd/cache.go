...
func init() { Register("cache", runCache) }
...
func runCache() error {
	inp := CacheInput{}
	if err := ac.GetJSONInput("cache_input", &inp); err != nil {
...
func restoreCache(inp CacheInput) error {
	...
	if resp.StatusCode == http.StatusNoContent {
		ac.Info(fmt.Sprintf("cache: miss for key %q", inp.Key))
		if err := ac.SetOutput("cache_hit", "false"); err != nil {
			ac.Warning(fmt.Sprintf("could not set cache_hit: %v", err), nil)
		}
		if inp.FailOnMiss {
	...
	ac.Info(fmt.Sprintf("cache: hit — matched key %q", entry.CacheKey))
	...
	if err := ac.SetOutput("cache_hit", "true"); err != nil {
		ac.Warning(fmt.Sprintf("could not set cache_hit: %v", err), nil)
	}
...
func saveCache(inp CacheInput) error {
	...
	ac.Info(fmt.Sprintf("cache: archiving %v", inp.Path))
	archivePath, archiveSize, err := createTarGz(inp.Path)
	if err != nil {
		return fmt.Errorf("creating cache archive: %w", err)
	}
	defer os.Remove(archivePath)
	ac.Info(fmt.Sprintf("cache: archive size %d bytes", archiveSize))
	...
	if resp.StatusCode == http.StatusConflict {
		ac.Info(fmt.Sprintf("cache: key %q already exists, skipping save", inp.Key))
		return nil
	}
	...
	ac.Info(fmt.Sprintf("cache: saved %q", inp.Key))

#!/bin/bash
# bootstrap.sh — downloads the docker binary from GitHub Releases,
...
REPO="js-fatigue/gha"
BINARY_NAME="docker"
CACHE_DIR="${RUNNER_TEMP:-/tmp}/actions/docker"
...
# Allow pinning via GHA_DOCKER_VERSION env var; default to latest
RELEASE_TAG="${GHA_DOCKER_VERSION:-latest}"

BINARY_PATH="${CACHE_DIR}/${RELEASE_TAG}/${BINARY_NAME}"

# Serve from filesystem cache if available
if [[ -x "$BINARY_PATH" ]]; then
  echo "docker: using cached binary at ${BINARY_PATH}"
...
if [[ -n "${ACTIONS_RESULTS_URL:-}" && -n "${ACTIONS_RUNTIME_TOKEN:-}" ]]; then
  ...
  if [[ -n "$ARCHIVE_URL" ]]; then
    echo "docker: cache hit — restoring binary"
    mkdir -p "$(dirname "$BINARY_PATH")"
    if curl -sL "$ARCHIVE_URL" | tar -xz -C / 2>/dev/null && [[ -x "$BINARY_PATH" ]]; then
      exec "$BINARY_PATH" "$COMMAND"
    fi
    echo "docker: cache restore failed, falling back to download"
  fi
fi
...
echo "docker: downloading ${ASSET_NAME} (release: ${RELEASE_TAG})"
...
if [[ -z "$EXPECTED" ]]; then
  echo "docker: ${ASSET_NAME} not found in checksums.txt" >&2
  rm -f "$BINARY_PATH"
  exit 1
fi
ACTUAL=$(sha256sum "$BINARY_PATH" | cut -d' ' -f1)
if [[ "$EXPECTED" != "$ACTUAL" ]]; then
  echo "docker: checksum mismatch for ${ASSET_NAME}" >&2
  echo "  expected: ${EXPECTED}" >&2
  echo "  actual:   ${ACTUAL}" >&2
  rm -f "$BINARY_PATH"
  exit 1
fi
echo "docker: checksum OK"

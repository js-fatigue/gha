#!/bin/bash
# bootstrap.sh — downloads the aws binary from GitHub Releases,
...
REPO="js-fatigue/gha"
BINARY_NAME="aws"
CACHE_DIR="${RUNNER_TEMP:-/tmp}/actions/aws"
...
# Allow pinning via GHA_AWS_VERSION env var; default to latest
RELEASE_TAG="${GHA_AWS_VERSION:-latest}"

BINARY_PATH="${CACHE_DIR}/${RELEASE_TAG}/${BINARY_NAME}"

# Serve from filesystem cache if available
if [[ -x "$BINARY_PATH" ]]; then
  echo "aws: using cached binary at ${BINARY_PATH}"
...
if [[ -n "${ACTIONS_RESULTS_URL:-}" && -n "${ACTIONS_RUNTIME_TOKEN:-}" ]]; then
  ...
  if [[ -n "$ARCHIVE_URL" ]]; then
    echo "aws: cache hit — restoring binary"
    mkdir -p "$(dirname "$BINARY_PATH")"
    if curl -sL "$ARCHIVE_URL" | tar -xz -C / 2>/dev/null && [[ -x "$BINARY_PATH" ]]; then
      exec "$BINARY_PATH" "$COMMAND"
    fi
    echo "aws: cache restore failed, falling back to download"
  fi
fi
...
echo "aws: downloading ${ASSET_NAME} (release: ${RELEASE_TAG})"
...
if [[ -z "$EXPECTED" ]]; then
  echo "aws: ${ASSET_NAME} not found in checksums.txt" >&2
  rm -f "$BINARY_PATH"
  exit 1
fi
ACTUAL=$(sha256sum "$BINARY_PATH" | cut -d' ' -f1)
if [[ "$EXPECTED" != "$ACTUAL" ]]; then
  echo "aws: checksum mismatch for ${ASSET_NAME}" >&2
  echo "  expected: ${EXPECTED}" >&2
  echo "  actual:   ${ACTUAL}" >&2
  rm -f "$BINARY_PATH"
  exit 1
fi
echo "aws: checksum OK"
